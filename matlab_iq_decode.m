...
% MATLAB(R) can be used to acquire I/Q eNodeBOutput from a wide range of
...
% MHz, 5 MHz, 10 MHz. The captured eNodeBOutput is stored in the file
% eNodeBOutput.mat.
%
...
loadFromFile = true; % Set to 0 to generate the eNodeB output locally
...
if loadFromFile
    ...
    rmc.SIB.VRBLength = 8;
    rmc.SIB.Gap = 0;
    % SIB eNodeBOutput field filled with random bits, this is not a valid SIB
    % message
    rmc.SIB.eNodeBOutput = randi([0 1],176,1);
...
% lteOFDMInfo configured for 6 resource blocks. enb.CNyclicPrefix is set
...
% have enough eNodeBOutput, terminate otherwise. 
...
% While we have more eNodeBOutput left, attempt to decode SIB1
